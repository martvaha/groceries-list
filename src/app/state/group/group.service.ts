...
export class GroupService2 {
  ...
  getGroups() {
    return combineLatest(this.store.select(selectActiveListId), this.store.select(selectGroupMaxModified)).pipe(
  ...
  addGroup(group: Group, listId: string) {
    ...
    return this.db
      .doc(`lists/${listId}/groups/${slug}`)
      .set({ ...others, modified: firebase.firestore.FieldValue.serverTimestamp() });
  }
  ...
  private extractGroup(change: DocumentChangeAction<Group>) {
    ...
    const group = { id, ...data, modified } as Group;
    return group;
  }
}

...
export class DurationPipe implements PipeTransform {
  transform(value: unknown, now: number = Date.now()): string | null {
    ...
    if (hours >= 1) {
      return hours === 1
        ? $localize`:@@duration.hour:1 hour`
        : $localize`:@@duration.hours:${hours}:INTERPOLATION: hours`;
    }

    const minutes = Math.max(1, differenceInMinutes(nowDate, date));
    return minutes === 1
      ? $localize`:@@duration.minute:1 minute`
      : $localize`:@@duration.minutes:${minutes}:INTERPOLATION: minutes`;
  }
}
